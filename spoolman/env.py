...
def get_backups_dir() -> Path:
    ...
    try:
        ...
        if Path("/.dockerenv").exists():
            logger.warning(f"Cannot create backups directory {backups_dir}, skipping (macOS Docker bind mount issue)")
        else:
            raise
    return backups_dir
...
def check_write_permissions() -> None:
    ...
    if Path("/.dockerenv").exists():
        logger.warning("Running in Docker, skipping write permission check")
        return

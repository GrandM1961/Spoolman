...
export const FilamentShow = () => {
  ...
  return (
    ...
    >
      ...
      />
      <Title level={5}>{t("filament.fields.settings_extruder_min_temp")}</Title>
      {!record?.settings_extruder_min_temp ? (
        <TextField value="Not Set" />
      ) : (
        <NumberFieldUnit value={record?.settings_extruder_min_temp ?? ""} unit="°C" />
      )}
      <Title level={5}>{t("filament.fields.settings_extruder_max_temp")}</Title>
      {!record?.settings_extruder_max_temp ? (
        <TextField value="Not Set" />
      ) : (
        <NumberFieldUnit value={record?.settings_extruder_max_temp ?? ""} unit="°C" />
      )}
      <Title level={5}>{t("filament.fields.settings_bed_min_temp")}</Title>
      {!record?.settings_bed_min_temp ? (
        <TextField value="Not Set" />
      ) : (
        <NumberFieldUnit value={record?.settings_bed_min_temp ?? ""} unit="°C" />
      )}
      <Title level={5}>{t("filament.fields.settings_bed_max_temp")}</Title>
      {!record?.settings_bed_max_temp ? (
        <TextField value="Not Set" />
      ) : (
        <NumberFieldUnit value={record?.settings_bed_max_temp ?? ""} unit="°C" />
      )}
